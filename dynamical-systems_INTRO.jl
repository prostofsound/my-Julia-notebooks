...
#> date = "2025-09-02"
#> tags = ["physics", "dynamical systems", "julia"]
#> description = "These notes are bases on the book \"Nonlinear Dynamics: A Concise Introduction Interlaced with Code - George Datseris, Ulrich Parlitz\""
#> 
#>     [[frontmatter.author]]
#>     name = "Silva Alejandro"
#>     url = "https://github.com/prostofsound"
...
# ╔═╡ 8a86aa5d-096c-43b1-9709-9a394363b741
...
# ╔═╡ 7d6743be-4bab-463b-8d1b-be48a9af5ab3
md"""
 # What's a Dynamical System?
It's a system described by a set of variables whose values change with time according to some predefined rules. We try to predict its future and understand the limitations.
"""

# ╔═╡ 0438837a-cb59-4662-91fe-0f78aa9e3ace
html"""
<img src="https://www.sfu.ca/~rpyke/335/fractal1.jpg" width="400">
"""

# ╔═╡ f3cb3e4e-b96e-4961-82e1-353e0ea7d58e
md"""
## State variables
They provide enough information about the system and constitute the **state vector**:  
  $x = (x_1, x_2, \dots, x_D)$, each component of $x$ is a real number.

- The space that **x** occupies is called the **phase space**, which has dimension *D*.

## Classification
- **Discrete dynamical systems**
$\textbf{x}_{n+1} = f(\textbf{x}_n)$
where _f_ is the dynamical rule.

- **Continuous dynamical systems**
$\dot{\mathbf{x}} = \frac{d\mathbf{x}}{dt} = f(\mathbf{x})$
Continuous systems are defined by a set of coupled ODEs. All expressions with higher order derivatives can be written as first-order systems by introducing a change of variable.

_In discrete D.S $f$ provides the direct change, while in continuous D.S $f$ provides the rate of change_.

## Trajectories, Uniqueness and Nonlinearity
- **_Trajectory_**
Represents the evolution from an intial condition of the state vector, in **phase space**.

-- Fixed points, where $f(\textbf{x}*) = \textbf{x}*$ or $f(\textbf{x}*) = 0$ is a state that never changes in time.

-- Periodic trajectories are a finite number of points (discrete case) or a closed curve (continuous case) in **state space**.

![](https://www.maplesoft.com/view.aspx?SI=4855/HamSys_24.gif)

- **_Uniqueness Theorem_**
The Picard-Lindelof theorem states that for each initial condition $\textbf{x}_0$ a unique solution $\textbf{x}(t)$ of the ODEs exist for t $\in (- \infty, \infty)$ if the vector field _f_ fulfills the Lipschitz continuity condition.

- **_Nonlinearity_**
The superposition principle is not satisfied (nature at work).

![](https://upload.wikimedia.org/wikipedia/commons/thumb/6/6b/Panorama20a_%286017119393%29.jpg/1920px-Panorama20a_%286017119393%29.jpg)
"""

# ╔═╡ 6d84ba9a-668d-11f0-2c49-e71874c20b0f
md"""
# Popular Dynamical Systems
"""

# ╔═╡ 1439564b-5da8-4099-bcef-1ccd2ce3dd3e
md"""
## Henon map
```math
\begin{aligned}
...
# ╔═╡ b311581b-f804-4969-ad02-3caccf6a94a2
...
# ╔═╡ 608be404-5131-4802-b413-372b2ea55264
begin
	# Then, we define initial state and parameters
	u00 = [0.2, 0.3]
	p0 = [1.4, 0.3] #parameters a,b respectively

	# Lastly, we give these three to the `DeterministicIteratedMap`:
	henon = DeterministicIteratedMap(henon_rule, u00, p0)
end

# ╔═╡ a21cae0e-b6b0-44d7-b2f4-b706b63971f4
md"""
`henon` is a **`DynamicalSystem`**, one of the two core structures of the _`DynamicalSystems`_ package.
They can be evolved interactively.
The simplest thing you can do with a _`DynamicalSystem`_ is to get its trajectory:
"""

# ╔═╡ 74bb561f-36ad-433c-b51b-4cc39f511362
begin
	total_time = 10000 
	X, t0 = trajectory(henon, total_time)
	scatter(X)
end

# ╔═╡ dafc754f-2af6-4132-acc0-a7f06ba16c92
md"""
The horizontal axis is the parameter _`a`_ and the vertical axis corresponds to _`b`_.
`X` is a **`StateSpaceSet`**, the second of the core structures of the library.
"""

# ╔═╡ dc3e6307-080a-4092-b68d-cee431a94e24
md"""
## Lorenz-96 system
```math
\frac{dx_i}{dt} = (x_{i+1}-x_{i-2})x_{i-1} - x_i + F
```
for $i \in \{1, \ldots, N\}$ and $N+j=j$. Here, instead of a discrete time map we have $N$ coupled ordinary differential equations.
"""

# ╔═╡ 9a27a271-649f-4601-abd5-c5b239737dab
function lorenz96_rule!(du, u, p, t) #to overwrite in place the rate of change in a pre-allocated container
    F = p[1]; N = length(u)
    ## 3 edge cases, necessary to define the system of ODES
    du[1] = (u[2] - u[N - 1]) * u[N] - u[1] + F
    du[2] = (u[3] - u[N]) * u[1] - u[2] + F
    du[N] = (u[1] - u[N - 2]) * u[N - 1] - u[N] + F
    ## Now, we write the general case
    for n in 3:(N - 1)
...
# ╔═╡ b36b8524-8ad9-4366-a721-6dcee507b842
# Initial condition and initialize the system
begin
	N = 5
	u01 = range(0.1, 1; length = N)
	p01 = [8.0]
	lorenz96 = CoupledODEs(lorenz96_rule!, u01, p01)
end

# ╔═╡ c38dc9ab-83d7-48f1-84a7-55b15dba2e42
# Plotting the time series
begin
	total_time1 = 12.5
	sampling_time = 0.02
	Y, t21 = trajectory(lorenz96, total_time1; Ttr = 2.2, Δt = sampling_time)
	Y

	# We can't scatterplot something 6-dimensional but we can visualize all timeseries
	fig0 = Figure(;size = (1000, 500))
	ax0 = Axis(fig0[1, 1]; xlabel = "time", ylabel = "variable")
...
# ╔═╡ 23affad0-6ec6-4b94-b5b3-c001af4cf994
md"""
A time series is not a trajectory. For a time series, we're looking the evolution of the system in time and a trajectory is defined in phase space (where the axes correspond to state variables).
"""

# ╔═╡ 0aa70c7a-52e4-4b06-9bac-dda6f31f3bba
length(columns(Y))

# ╔═╡ 10a8124c-0db9-4d18-95ae-d222a36009ac
md"""
## Lorenz-63 system
```math
\begin{aligned}
...
\end{aligned}
```
The equations relate the properties of a two-dimensional fluid layer uniformly warmed from below and cooled from above. $x$ is proportional to the rate of convection, $y$ to the horizontal temperature variation, and $z$ to the vertical temperature variation. The constants σ, ρ, and β are system parameters.
"""

# ╔═╡ 52a8b62e-43e9-4ff7-9abf-b2023607d56c
function lorenz_rule(u, p, t)
        # Unpacking
        σ, ρ, β = p #parameters
        x, y, z = u #initial conditions
        ...
        return SVector(dx, dy, dz) #return static vector
end

# ╔═╡ 03fe251c-0dfd-43f1-ab1f-925b7fc87f0a
begin
	# Test
	p = [10.0, 28.0, 8/3]
	u0 = @SVector [0.0, 10.0, 0.0]

	# Create an instant of a "DynamicalSystem"
	lorenz = ContinuousDynamicalSystem(lorenz_rule, u0, p)

	T = 100.0 # total time
	Δt = 0.01 # sampling time
	A, times = trajectory(lorenz, T; Δt) # evolving the I.C, CAREFUL WITH KEYWORDS!

	# A is a matrix: each row is [x, y, z] at a time
	# Plotting
    fig1 = Figure(; size = (600, 600))
    ax1 = Axis3(fig1[1, 1],
        xlabel = "x", ylabel = "y", zlabel = "z",
        title = "Lorenz attractor",
        azimuth = 310, elevation = 50
    )

    lines!(ax1, A[:, 1], A[:, 2], A[:, 3], color = :dodgerblue)
    fig1
end

# ╔═╡ c7df1f20-5337-4fbd-bcc6-23d440de2c96
md"""
# Stability of fixed points

Let's take the continuous case. A fixed point satisfies $f(\mathbf{x}^{*}) = 0$.  
We're interested in the dynamics of $\mathbf{x} = \mathbf{x}^{*} + \mathbf{y}$, where $\mathbf{y}$ is an infinitesimal amount.

- We can linearize the dynamics with respect to $\mathbf{x}^{*}$ by using a Taylor expansion:
$\mathbf{x}^* + \mathbf{y} = f(\mathbf{x}^*) + J_f(\mathbf{x}^*) \cdot \mathbf{y},$
...
## Remarks
- As __y__ increases nonlinear effects become relevant.
- We're interested in the **eigenvalues** and **eigenvectors** of the Jacobian.
- If the eigenvalues ($\mu_i$) are real, then they represent the rate of change of the perturbation along the corresponding eigenvector. If imaginary,
$\begin{align*}
\text{Re}(\mu_i) &< 0 \text{ or } |\mu_i| < 1 \Rightarrow \mathbf{x}^* \text{ is stable.} \\
\text{Re}(\mu_i) &> 0 \ \text{(at least one)} \text{ or } |\mu_i| > 1 \Rightarrow \mathbf{x}^* \text{ is unstable.}
\end{align*}$

## Dissipative systems
Almost for all initial values __x__, the volume _v_ of an infinitesimally small ball of initial conditions following the trajectory starting at __x__ will converge to zero as $t \rightarrow \infty$.

- However, the phase space of some systems is populated by _attractors_: invariant subsets of the phase space that attract nearby initial conditions towards them. These are called dissipative systems.
- **Attractors** are defined as sets of zero volume in phase space.
"""

# ╔═╡ 945d06df-18fd-4031-9ad5-3cc40f5eb002
md"""
# 1D Continuous Systems
Suppose we want to model the temperature of the Earth. We consider the following factors:

- Incoming solar radiation ($S_0$ is the solar constant).
- The outgoing terrestrial radiation.

$\begin{equation}
c \frac{dT}{dt} = \frac{S_{0}}{4} \left( 1 - \alpha(T) \right) - \epsilon \sigma T^{4}
\end{equation}$ $\alpha$ is the planetary albedo, which is the fraction of the total radiation reflected by the Earth.

## Preparing the equations
The objective is to **separate the dynamical context from the physical**. 

- We group and reduce unnecesary parameters.
- De-dimensionalize quantities (remove the physical units) based on suitable scales.
- In most cases, its the ratio between two physical parameters that matters.
...
# ╔═╡ e150a971-a7e2-490d-b0a4-d4878f06a5d8
md"""
# 2D Continuous Systems
There are only two eigenvalues (from the Jacobian matrix). 
"""

# ╔═╡ 12caabe4-7f0f-4502-9a00-a3cfcf6b0077
html"""
<a href="https://ibb.co/JRxzTHLq"><img src="https://i.ibb.co/PZT5ygpM/Screenshot-from-2025-08-21-22-17-37.png" alt="Screenshot-from-2025-08-21-22-17-37" border="0"></a>
"""

# ╔═╡ 95243b0e-0040-4875-80f7-2cec4b40b76c
md"""
# Chaos in 1D, 2D
## Continuous Systems
**1D continuous systems** can only converge to a fixed point or diverge to infinity. 

### What about 2D continuous systems?

- The Poincaré-Bendixon theorem says the **phase space** can only have fixed points and periodic orbits. This is because solutions to the ODEs must be smooth and must not cross each other in state space.
- There is no enough room for the trajectories to wiggle around and create chaos. 

**For continuous dynamical systems, one needs at least 3 dimensions for chaos to arise.**

BUT, what about the double pendulum then?
The key point is that the theorem talks about dimensions in phase space. For the double pendulum we have $\theta_1,\theta_2, \dot{\theta}_1, \dot{\theta}_2$ in phase space. Then, there is no restriction for chaos to emerge.

**For n degrees of freedom, the phase space is 2n-dimensional.**

## Discrete Systems
Here, there is no limitation from the state space itself since there is no a continuous line connecting the points. Then, even in 1D chaos is present (logistic map, standar map, etc).

![](https://www.islandsoforder.com/uploads/4/3/4/1/43417789/editor/logisticmap.jpg?1495306180)
"""

# ╔═╡ e21df6d7-8517-4777-a4c9-d936603c78f9
md"""
# Defining and measuring Chaos
Tiny differences in the initial conditions of a system can lead to very different evolutions of the system. _It's not a sufficient condition for chaos, but it's necessary_.

- Decreasing the initial distance between the trajectories would not eliminate the effect. It would take longer to appear. 
- We're in theory able to predict everything about the system. However, the evolution of the system is non-periodic, apparently irregular, difficult to predict but still deterministic and full of patterns.
- This **structure in state space** distinguishes deterministic chaos from pure randomness.

## Largest Lyapunov Exponent
We can quantify the divergence of the trajectories by how fast they are evolving (exponentially fast).

- It occurs only for small distances between trajectories.
- After some time, they saturate and stop increasing. 

Lyapunov exponents quantify stretching (or contracting) of perturbations in characteristic directions. Adequate for linearized dynamics.

![](https://www.startpage.com/av/proxy-image?piurl=https%3A%2F%2Ftse4.mm.bing.net%2Fth%2Fid%2FOIP.EitcXthV46KxeFvUFMp4MQHaF7%3Fr%3D0%26pid%3DApi&sp=1756864031Tae69cb5287f51379f92d91ca9cf972f42914d6e93c9e3378784c750955ed38a5)

"""

# ╔═╡ c59d78b7-282a-4baa-a959-36c8dac48e80
md"""
# Predefined Dynamical Systems
"""

# ╔═╡ 6c651dd5-caa8-4fd7-b643-4b81e39547a8
md"""
### Lorenz-84 system
"""

# ╔═╡ 7cb6b194-0d4b-4acc-b283-e232084edbf8
begin
...
# ╔═╡ f0d921f6-98d3-4491-b408-e9ea0a76557b
# Select initial condition
@bind i PlutoUI.Slider(1:4; show_value=true)

# ╔═╡ 57ee14a4-9891-435a-bcd5-07f04ef31e6e
begin
	# Generate trajectory (returns a tuple)
	traj, t = trajectory(ds, 1000, u0s[i]; Δt = 0.05)

	# Plot trajectory
	fig2 = Figure(; size = (600, 400))
	ax2 = Axis(fig2[1, 1]; xlabel = "x", ylabel = "z")
	lines!(ax2, traj[:, 1], traj[:, 3], color = :crimson)

	fig2
end

# ╔═╡ b284b270-6914-4a46-923b-046a6131a537
md"""
### Double pendulum
"""

# ╔═╡ e7f1335f-2d78-433c-a1ea-52098f18a6cc
# Define the system and initial conditions
begin
	ds1 = PredefinedDynamicalSystems.double_pendulum()

	# Initial conditions
	u11 = [π/2, 0.0, 0.08, 0.5]                  # symmetric, nearly stable
	u22 = u11 .+ 5e-3                            # perturbed
	u33 = [π/2, 0.1, 1.0, -0.5]                	# chaotic regime
	u44 = [π/2, π/4, 0.5, -0.5] .+ 2e-3         # slightly different chaotic

	u0s1 = [u11, u22, u33, u44]
end

# ╔═╡ da1119d4-cbf0-41a4-a3a7-bb5a3cfd30be
# Interactive slider to choose initial conditions
@bind i1 PlutoUI.Slider(1:4; show_value=true)

# ╔═╡ 17610603-4288-448d-a20b-247607cb59b3
begin
	# Generate trajectory (returns a tuple)
	traj1, t1 = trajectory(ds1, 1000, u0s1[i1]; Δt = 0.05)

	# Plotting
	fig3 = Figure(; size = (1000, 700))
	ax3 = Axis(fig3[1, 1]; xlabel = L"\dot{\theta}_1", ylabel = "θ2", title = L"\dot{\theta}_1 vs\ \theta_2", titlesize = 24)
	lines!(ax3, traj1[:, 1], traj1[:, 4], color = :black)

	fig3
end

# ╔═╡ 3fad4bcd-c358-4857-a029-31542c0395f0
md"""
### Guckenheimer-Holmes
```math
\begin{aligned}
...
# ╔═╡ 0b369f3c-d8f1-4476-bbe6-981c294370d7
begin
	# Define system with parameters
	ds2 = PredefinedDynamicalSystems.guckenheimer_holmes(
	    a = 0.4,
	    b = 20.25,
	    c = 3,
	    d = 1.6,
	...
	# Initial condition
	ic_2 = [-0.55582369, 0.0514, 0.98]
end

# ╔═╡ b6336de8-f508-4e73-950f-cfe7e34ec093
begin
    # Generate trajectory with initial condition `ic_2`
    traj2, t2 = trajectory(ds2, 300, ic_2; Δt = 0.05)

    # Plotting with CairoMakie
    fig4 = Figure(;size = (1000, 700))
    ax4 = Axis(fig4[1, 1], xlabel = "z", ylabel = "y")
    lines!(ax4, traj2[:, 3], traj2[:, 2], color = :green)

    fig4
end

# ╔═╡ c6c4ab7a-eb95-4ac3-baf2-bf70e00614b3
md"""
# Predictability Horizon
Suppose we carry out an experiment. The evolution of the state vector of a system (defined from a measurement) is much faster than it would occur in reality.

- But, our measurement isn't perfect. It has an initial distance from the real state of the system.
- If the system is chaotic, both trajectories will diverge from each other exponentially.
- We can define a time (Lyapunov time) when the tolerance for this separation is reached.

Any prediction will eventually become unusable.

"""

# ╔═╡ ca29f94c-dece-4fc0-a121-8a8f26d4deb8
md"""
# Bifurcations
They're the significant qualitative changes the evolution of system experiences as a consequence of varying its parameters. Also, the stability of fixed points can change during these changes.
"""

# ╔═╡ 76477eb4-0b8c-437c-8494-bd13d764fbbe
begin
    # Initialize the system
    ro = Systems.roessler()  # The initial conditions are embedded in `ro`
    cs = 1.5:0.01:5.2
    plane = (2, 0.0)  # Hyperplane for the Poincaré section
    
    # Initialize an empty list to store all the points
    all_psos = []
    
    # Loop through the values of c and collect the data
    for c in cs
        set_parameter!(ro, 3, c)
        
        # Call poincaresos
        psos = poincaresos(ro, plane, 10000.0; Ttr = 2000)
        
        # Add the data points to the list
        append!(all_psos, [(fill(c, length(psos)), psos[:, 1])])
    end
    
    # Flatten the collected data
    flattened_c = vcat([data[1] for data in all_psos]...)
    flattened_psos = vcat([data[2] for data in all_psos]...)
    
    # Plot using CairoMakie
    figure_plot, ax_plot = scatter(flattened_c, flattened_psos, 
                                   markersize = 0.2, color = :blue, alpha = 3)
    
    # Display the figure
    figure_plot
end

# ╔═╡ 4f4fa0d5-aab1-4d77-aaa1-1549db616cd2
md"""
...
manifest_format = "2.0"
project_hash = "8b0700c0c9e379f2744b37986a33d98e266ef719"
...
git-tree-sha1 = "284591a71dbef07f43236e6b28404a17f2c0fb8f"
...
git-tree-sha1 = "f663ea30034b8a6d69ae96eaf7a26d5291e97c46"
...
deps = ["Attractors", "ChaosTools", "ComplexityMeasures", "DataStructures", "DelayEmbeddings", "DynamicalSystemsBase", "FractalDimensions", "PeriodicOrbits", "PredefinedDynamicalSystems", "RecurrenceAnalysis", "Reexport", "SignalDecomposition", "StateSpaceSets", "TimeseriesSurrogates"]
git-tree-sha1 = "b50a5324873023f11ab6a1ede4c48b7b0b888980"
...
git-tree-sha1 = "f7992102edfc6572f021050ad5ab321a6853d327"
...
git-tree-sha1 = "55e4a13c1ed32bba37e9cbcbe3ee808de82c403e"
...
git-tree-sha1 = "1045bdc618b74338c546b20e285aa6b75c165386"
